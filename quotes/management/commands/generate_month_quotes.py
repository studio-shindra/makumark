...
class Command(BaseCommand):
    ...
    def _fetch_quote_from_openai(self, client: OpenAI, playwright: dict) -> dict:
        ...
        system_msg = (
            ...
            "日本語テキストについては「翻訳」を行ってはいけません。"
            "既存の翻訳家が作った日本語訳（例として有名な定訳など）と同じ、"
            "あるいは非常によく似た表現を使わないでください。"
            "原文の意味やニュアンスを踏まえつつ、"
            "新しく書き起こした解釈文・意訳・要約としての日本語文を生成してください。"
        )
...
- 日本語テキストは「翻訳」ではなく、原文の意味を噛み砕いた
  「新しい解釈文・意訳・要約」として書いてください。
- 「生きるべきか死ぬべきか、それが問題だ。」のような、
  すでに広く知られた日本語訳と同じ、あるいは酷似した表現は使わないでください。
- 可能であれば原文（英語など）も出してください。
